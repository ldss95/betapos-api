		...
		it('Deberia actualizar el nombre del rol creado', done => {
			http(app)
				...
				.send({
					...
					description: 'Este rol es creado por los tests unitarios, deberia ser eliminado automaticamente (Actualizado)',
					id: roleId
				})
				.set('Authorization', `Bearer ${token}`)
				.set('Cookie', session)
		...
		it('Deberia fallar al intentar actualizar rol inexistente', done => {
			http(app)
				...
				.send({
					...
					descrption: 'Este rol es creado por los tests unitarios, deberia ser eliminado automaticamente (Falla - Actualizar)',
					id: 'inexistent-role'
				}).expect(404, done)
		})
	})
